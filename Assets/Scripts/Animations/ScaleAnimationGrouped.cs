...
public class ScaleAnimationGrouped : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            float newScale = initialScale[i].y + amplitudeY*0.25f * Mathf.Sin(2 * Mathf.PI * frequencyY * timeElapsed + Mathf.PI/2 );
            float newScalex = initialScale[i].x + amplitudeX * Mathf.Sin(2 * Mathf.PI * frequencyX * timeElapsed - Mathf.PI / 2f);
